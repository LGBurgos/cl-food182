...
class MergeSaleOrder(models.TransientModel):
    ...
    merge_type = fields.Selection([('new_cancel', 'Create new quotation and cancel all selected quotation'),
                                   ...
                                   (
                                       'merge_delete',
                                       'Merge quotation on existing selected quotation and delete others')],
                                  default='new_cancel')
    sale_order_id = fields.Many2one('sale.order', 'Sale Order')
    customer_id = fields.Many2one(
        'res.partner',
        string='Customer',
         # Use the dynamically computed partner_ids
    )
    dynamic_partner_ids = fields.Many2many(
        'res.partner',
    ...
    @api.depends('merge_type')
    def _compute_partner_ids(self):
        """Compute partner_ids dynamically based on active sale orders."""
        for record in self:
            sale_orders = record.env['sale.order'].browse(record._context.get('active_ids', []))
            partner = sale_orders.mapped('partner_id').ids
            self.dynamic_partner_ids = [(6,0,partner)]
    ...
    def merge_orders(self):
        ...
        if len(self._context.get('active_ids', [])) < 2:
            raise UserError(_('Please select at least two sale orders to perform the Merge Operation.'))

        if any(order.state not in ['draft', 'sent'] for order in sale_orders):
            raise UserError(
                _('Please select Sale orders which are in Quotation stage to perform the Merge Operation.'))
        ...
        if self.merge_type == 'merge_cancel' or self.merge_type == 'merge_delete':
            ...
            processed_products = {}
            sale_order_id.write({'merge_date': datetime.now(),
                                 'merge_user_id': self.env.user.id,
                                 'merge_sale_orders': ",".join(sale_orders.mapped('name')),
                                 'is_merge_order': True, })
